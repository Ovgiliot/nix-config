...
-- All plugins are declared in Nix modules (core neovim.nix + workflow modules).
-- This file only configures them — no plugin manager, no lazy specs.
-- Workflow-specific sections are guarded with pcall so missing plugins
-- (when a workflow is not active) do not cause errors.
...
-- Development groups (conditional — only if neogit is installed)
if pcall(require, "neogit") then
	table.insert(wk_groups, { "<leader>a", group = "ai" })
	table.insert(wk_groups, { "<leader>c", group = "code" })
	table.insert(wk_groups, { "<leader>g", group = "git" })
end

-- Note-taking groups (conditional — only if orgmode is installed)
if pcall(require, "orgmode") then
	table.insert(wk_groups, { "<leader>m", group = "org-mode" })
	table.insert(wk_groups, { "<leader>o", group = "org" })
	table.insert(wk_groups, { "<leader>r", group = "roam" })
end
...
-- ===========================================================================
-- WORKFLOW: Note-taking (orgmode, org-roam, headlines)
-- ===========================================================================
do
	...
	local ok_roam, roam = pcall(require, "org-roam")
	if ok_roam then
		roam.setup({
					...
					attach_mappings = function(prompt_bufnr)
						actions.select_default:replace(function()
							...
							if sel then
								...
								local line =
									vim.api.nvim_buf_get_lines(bufnr, cursor[1] - 1, cursor[1], false)[1] or ""
								local col = cursor[2]
	...
	local ok_hl, headlines = pcall(require, "headlines")
	if ok_hl then
		headlines.setup({
			org = {
				headline_highlights = {
...
-- ===========================================================================
-- WORKFLOW: Development (neogit, copilot, opencode, LSP, cmp, conform, lint, dap)
-- ===========================================================================
do
	-- Git -----------------------------------------------------------------------
	...
	-- Copilot -------------------------------------------------------------------
	-- copilot-vim is a Vimscript plugin that auto-loads from plugin/.
	-- Disable its default Tab mapping unconditionally (no-op when absent),
	-- then set our own accept keymaps after VimEnter when the autoload
