...
if [[ "$PROFILE" == "server" ]]; then
	if [[ "$UNATTENDED" == true ]]; then
		...
		warn "Change it immediately after login with: passwd ethel"
		while true; do
			prompt_secret INITIAL_PASSWORD "Initial password for ethel"
...
# ---------------------------------------------------------------------------
if [[ "$SKIP_INSTALL" != true ]]; then
	info "Copying dotfiles to /mnt/home/ethel/dotfiles/nix ..."
	sudo mkdir -p /mnt/home/ethel/dotfiles
	sudo cp -rT "$FLAKE_DIR" /mnt/home/ethel/dotfiles/nix
	...
	sudo find /mnt/home/ethel/dotfiles/nix -maxdepth 1 -name 'result*' -type l -delete
	# ethel uid=1000 gid=1000 (standard NixOS first user).
	sudo chown -R 1000:1000 /mnt/home/ethel/
	ok "Dotfiles copied."
fi
