...
if [[ "$LIVE_ISO" == true ]]; then
	...
	# --- LUKS passphrase (all partitions always encrypted) ------------------
...
if [[ "$LIVE_ISO" == true ]]; then
	...
	# ------------------------------------------------------------------
	# Capture swap LUKS UUID for laptop hibernate support.
	# ------------------------------------------------------------------
	...
	if systemd-cryptenroll --tpm2-device=list 2>/dev/null | grep -q .; then
		...
		if [[ "$ENROLL_TPM" == "yes" ]]; then
			...
			# Home disk (two-disk only — always encrypted with same passphrase)
			if [[ "$SEPARATE_HOME" == true ]]; then
