...
- `flake.nix` — Entry point. Defines inputs and exposes `mkNixosHost` / `mkDarwinHost` helpers. Hosts are registered in `nixosConfigurations` / `darwinConfigurations`. Also defines `checks` (eval, build, integrity, format, VM test) and `formatter` (alejandra).
- `profiles/` — Opinionated machine archetypes. Import one profile per host; do not import individual modules directly from a host.
  ...
  - `workstation.nix` — Core + desktop + gaming + virtualization (no laptop power management or wallpaper).
...
- `modules/system/` — NixOS system modules, one concern per file.
  - `core/` — `boot.nix`, `nix.nix`, `locale.nix`, `networking.nix`, `security.nix`
  - `desktop/` — `audio.nix`, `display.nix`, `input.nix`
  - `laptop/` — `boot.nix`, `power.nix`, `services.nix`
  - `optional/` — `gaming.nix`, `virtualization.nix`
